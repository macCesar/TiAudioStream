...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "ti.audiostream TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "ti.audiostream TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
Java_ti_audiostream_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.audiostream", &TiModuleBinding);
