...
### Added
- **Android Auto: resume-on-connect through the official recents root.** When the car requests its recents root (`BrowserRoot.EXTRA_RECENT`) and a last station exists, the service now returns a dedicated root that exposes exactly one playable "resume last station" item, and it handles the `onPrepare` / `onPrepareFromMediaId` resumption handshake. On a service recreation (car reconnects with the app's JS not alive) the last station's playback fields are rehydrated from persistence, so a bare `onPlay` from the head unit resumes the last station instead of doing nothing. `ACTION_PLAY_FROM_MEDIA_ID` and the prepare actions are now advertised in the playback state.
- **iOS/CarPlay: next/previous now advance the station list natively (parity with Android).** When the head unit sends next/prev and the app's JS isn't alive to handle the `remotecontrol` event, the module advances through the persisted CarPlay station list itself — matching the current station by `streamUrl` with an `id` fallback — and plays the adjacent station (with wraparound), mirroring Android's `playAdjacentAutomotiveStation`. Apps that keep a `remotecontrol` listener handle it in JS as before; there is no new public API and `automotivestationselected` still syncs the phone UI.

### Fixed
- **iOS/CarPlay: the "On Air" mark can no longer land on two stations at once, or stick to the wrong one.** The list marked each row independently against two sources of truth — the persisted current station and the player's live URL — which disagree for a moment on every station switch (the new station is persisted before the player URL moves), so both the old and the new row could show as playing. The corrective refreshes ran at fixed delays (0.15/0.60/1.20s), so when the app's JS settled the state later than that — common when the app handles `automotivestationselected` itself — the stale double-mark stuck. The list now computes a single current row per refresh (priority: persisted URL → station `id` → live player URL → title), and `setStream` triggers a list re-mark whenever the playing URL actually changes, so the mark follows the real state instead of a timer. `setAutomotiveStations` also ignores re-publications of an identical list, and the CarPlay list now only performs a full template rebuild — which resets the car's scroll position — when the number of stations changes; content changes (reordered stations, new URLs, renamed programs) are applied in place, so the scroll no longer jumps when the app re-publishes its station list on a data refresh.
- **iOS: switching stations while paused now actually switches the station.** With playback paused, `pause()` clears `_playRequested`, so a following `setStream()` deferred preparing the new item — leaving the previous station's `AVPlayerItem` loaded. `start()` then saw a valid, ready item and just called `[_player play]`, resuming the *old* station; every later pause-and-switch kept replaying it. The module now records which URL the loaded item was built from and re-prepares whenever it no longer matches the current stream. Switching while playing was unaffected (and still is), since `setStream()` prepared eagerly in that path. Android was never affected — its service always re-prepares on `setStream`.
- **iOS/CarPlay: `setAutomotiveStations()` no longer rejects a single-station list.** With exactly one station, Titanium collapses `[station]` down to the bare dictionary; the argument-unwrap heuristic then pulled that lone dictionary out and the method rejected it as "expected array, got NSDictionary", so a one-station browse list was impossible without padding it with a second entry. The unwrap now only triggers when the sole argument element is itself an array, so a single-station list works directly.
- **Android Auto: the station list no longer flickers / resets its scroll on every song.** `updateMediaSessionMetadata` refreshed the whole browse tree (`notifyChildrenChanged`) on each metadata change, so during a live show the car's station list rebuilt itself — and lost its scroll position — every track. Track info travels through `MediaMetadata` + `PlaybackState`, not the browse tree, so that refresh was removed; the tree is still refreshed when the station list itself changes.
- **Android Auto: a foreground-service start that the OS refuses is no longer swallowed.** On Android 12+/14 the system can reject `startForeground` from the background (`ForegroundServiceStartNotAllowedException`); the service logged it and carried on, so the app never learned playback failed to start. It now fires an `error` state/event so the app can react.
- **Android Auto: a hung artwork host can no longer stall later artwork fetches.** `fetchArtworkFromUrl` had no connect/read timeouts and never disconnected, so a stalled server blocked the single shared background executor indefinitely. It now uses the same timeouts as the other artwork loader and always disconnects.

### Changed
- **Android Auto: skip next/previous is only advertised when there is more than one station.** With a single station the car no longer shows dead skip buttons.
- **Every on-screen string the module renders is now localizable through the host app's i18n, on both platforms.** Previously the CarPlay row said "Pausar"/"Reproducir" in hardcoded Spanish for every app regardless of its language, while Android hardcoded English for the notification buttons, the notification channel, the Android Auto browse items and the fallback title — so the same app spoke different languages on each platform, and neither followed the device. All 12 strings now resolve through `app/i18n/<lang>/strings.xml` (`ti_audiostream_*` keys), falling back to English when the app defines nothing. See the [Localization](README.md#localization) section for the full key list. **Android note:** an app whose only i18n locale is non-English must set `<defaultLang>` in `tiapp.xml`, otherwise Android drops those strings from the APK without warning and the module falls back to English.
